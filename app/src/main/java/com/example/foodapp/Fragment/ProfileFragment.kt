        ...
        val request = JsonObjectRequest(Request.Method.POST, url, jsonObject,
            { response ->
                ...
                if (status == "success") {
                    ...
                    if (response.has("id_user")) {
                        editor.putInt("userId", response.getInt("id_user")) // ✅ đúng key mà PHP trả
                    }
...
//                    Toast.makeText(requireContext(), "Cập nhật thất bại", Toast.LENGTH_SHORT).show()
                    Toast.makeText(requireContext(), "Mail này đã được sửa dụng.", Toast.LENGTH_SHORT).show()
